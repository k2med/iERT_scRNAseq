# ---------------------------
# Title: Fig.6 — CD8+ T UMAP, marker dotplots, heatmap, AUCell density, and TCR diversity
# Purpose:
#   - Visualize fine CD8+ T subsets by UMAP
...
# ---------------------------
# 2) UMAP colored by fine CD8+ T cell types (Fig.6A.umap)
# ---------------------------
...
# ---------------------------
# 3) Dotplots (Fig.6A, Fig.S6B) + source data
# ---------------------------
## 3.1 Fig.6A marker set
...
# ---------------------------
# 4) CD8+ T marker heatmap (Fig.S6A)
...
suppressWarnings({
  ComplexHeatmap::draw(
    top_markers_left(expr_mat, top_n = 5,
...
# ---------------------------
# 5) Fig.6B — AUCell programs: density on UMAP (Nebulosa)
# ---------------------------
...
auc_df <- data.frame(cell_id = colnames(auc_assay),
                     seu_cd8t_auc@reductions$umap@cell.embeddings,
                     t(auc_assay@data))
write.table(auc_df, "Fig6B_density_source_data.txt",
            sep = "\t", quote = FALSE, row.names = FALSE)
...
for (pg in program_order) {
  ...
  p_den <- Nebulosa::plot_density(
    seu_cd8t_auc, features = pg, reduction = "umap", size = 0.5, method = "wkde", adjust = 0.75
  ) +
...
# ---------------------------
# 6) Fig.S6C–D — TCR diversity (scRepertoire)
# ---------------------------
base_dir <- "../data/TCR"
