...
umap_all_df <- data.frame(cell_id = colnames(seu_all),
...
write.table(umap_all_df, "Fig2F_umap_source_data.txt",
            sep = "\t", quote = FALSE, row.names = FALSE)

umap_nk_df <- data.frame(cell_id = colnames(seu_nk),
                         seu_nk@reductions$umap@cell.embeddings,
                         coarse_cell_type = "NK cell",
                         group = seu_nk$group,
                         site = paste(
                           sapply(strsplit(as.character(seu_nk$orig.ident), "_"), `[`, 2),
                           sapply(strsplit(as.character(seu_nk$orig.ident), "_"), `[`, 3),
                           sep = "_"
                         )
)

write.table(umap_nk_df, "Fig2G_nk_umap_source_data.txt",
            sep = "\t", quote = FALSE, row.names = FALSE)

umap_cd8_df <- data.frame(cell_id = colnames(seu_cd8),
                          seu_cd8@reductions$umap@cell.embeddings,
                          coarse_cell_type = "CD8+ T cell",
                          group = seu_cd8$group,
                          site = paste(
                            sapply(strsplit(as.character(seu_cd8$orig.ident), "_"), `[`, 2),
                            sapply(strsplit(as.character(seu_cd8$orig.ident), "_"), `[`, 3),
                            sep = "_"
                          )
)

write.table(umap_cd8_df, "Fig2G_cd8t_umap_source_data.txt",
            sep = "\t", quote = FALSE, row.names = FALSE)
