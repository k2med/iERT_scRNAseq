# ---------------------------
# Title: Fig.3 — NK UMAP, marker dotplots, OR heatmap; Fig.4A; Fig.S3/S4
# Purpose:
#   - Visualize fine NK subsets (UMAP)
...
#   - Fig.S3B.pdf                        + FigS3B_dotplot_source_data.txt
...
# ---------------------------
# 1) Build UMAP on Harmony dimensions
# ---------------------------
dims_use <- 1:50
seu_nk   <- RunUMAP(seu_nk, reduction = "harmony", dims = dims_use, min.dist = 0.3)

# ---------------------------
# 2) UMAP colored by fine NK cell types (Fig.3A.umap)
# ---------------------------
...
# ---------------------------
# 3) Dotplot of NK marker genes (Fig.3A.dotplot)
# ---------------------------
...
# ---------------------------
# 4) Dotplot of NK function genes (Fig.S3C)
# ---------------------------
...
write.table(dot_df, "FigS3B_dotplot_source_data.txt",
            sep = "\t", quote = FALSE, row.names = FALSE)
...
# ---------------------------
# 6) Odds-ratio heatmap (Fig.3B) + source data
# ---------------------------
sample_levels <- c(
...
# ---------------------------
# 7) Fig.4A — Polar barplots of cytokines per group (+ per-group source data)
# ---------------------------
...
plot_polar <- function(df_one_group) {
  ggplot(df_one_group, aes(x = Cytokine)) +
    ...
    coord_polar() +
    geom_text(aes(y = es_max + 5, label = Cytokine, angle = angle),
              size = 5 / .pt) +
    ylim(c(es_min, es_max + 5)) +
    theme_void() +
    theme(
...
walk(prep_list, function(d) {
  ...
  ggsave(
    ...
    egg::set_panel_size(g, width = grid::unit(3, "cm"), height = grid::unit(3, "cm")),
...
# ---------------------------
# 8) Fig.S4A — GSVA heatmap of NK subsets (hallmark/GO/KEGG/REACTOME)
# ---------------------------
...
cluster_pathway_df <- data.frame(`Gene set` = rownames(cluster_pathway_mat_z),
                                 cluster_pathway_mat_z,
                                 check.names = FALSE)
write.table(cluster_pathway_df, "FigS4A_source_data.txt",
            sep = "\t", quote = FALSE, row.names = FALSE)
